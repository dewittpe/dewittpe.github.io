...
knitr::opts_chunk$set(collapse = TRUE,
                      fig.width = 4,
                      fig.height = 4,
                      fig.path = "figures/20161115-figures/") 
```
...
```
```{r new_save, include = FALSE}
base_plot_new <-
  base_plot +
  scale_x_continuous(breaks = unique(xi), labels = expr,
                     sec.axis = sec_axis(~ ., breaks = unique(xi)))
```
```{r plot_for_intro, echo = FALSE, fig.width = 7, fig.show = "hide", results = "hide"}
gridExtra::grid.arrange(base_plot_old + theme(legend.position = "none"),
                        base_plot_new + theme(legend.position = "none"),
                        nrow = 1)
```
